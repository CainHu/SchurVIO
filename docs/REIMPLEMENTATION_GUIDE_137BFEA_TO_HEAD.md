...
算法端点之后的 **3e7d6ed** 与本文件所在提交补充数学审计、文档和报告指标校正，不改变
ESKF/Schur 的估计结果；“当前最新”在本文中指**本文件所在提交的完整工程状态**，而估计器
行为基线仍由 ebab4a0 唯一标识。这样既避免文档自引用一个不可能预先写入的自身提交哈希，
又能精确区分算法变化与评估变化。
...
9. 实验性冗余感知关键帧删除；
10. 拆分后的源码、报告和数学专题；
11. 与左乘误差一致的联合 NEES、Q/P/V 协方差谱检查，以及初始化/删帧概率语义审计。
...
当前仿真用第一帧真值设置名义 \(R,p,v\)，但保留配置的初始协方差，bias 从零估计，重力
固定。这只隔离后端误差，不代表真实冷启动已经实现。第一条 IMU 只建立时间戳；视觉状态
必须先传播到相机曝光时刻再创建 clone。当前源码还把 `timestamp==0` 当作未初始化哨兵；
若日志从精确的 0 开始，重实现时应改用独立布尔标志，或保留非零时间原点。

重实现时先记录：

- 哪些名义量来自真值、静止初始化或外部先验；
- 对应 \(P_0\) 是否反映真实初始化精度；
- IMU、相机和真值是否同时间；
- `ESTIMATE_GRAVITY=false` 是否真的有已知重力方向和初始姿态作前提。

### 2.6 评估坐标不变量

左乘姿态协方差 \(P_{\theta\theta}\) 位于世界系切空间。若报告使用 `estimate-truth`，姿态
误差必须是

\[
e_\theta=\operatorname{Log}(R_{est}R_{gt}^T),
\]

才能和 \(e_p=p_{est}-p_{gt}\)、\(e_v=v_{est}-v_{gt}\) 以及
\(P_{\theta p},P_{\theta v}\) 一起构造联合 NEES。只比较旋转角时左右误差范数相同，但联合
统计的交叉项不允许混坐标系。
...
- Helix-3D：三轴激励；
- Stop-go：静止、弱视差和起停；
- Rotation-translation：纯旋转与平移解析分段，覆盖 RR/RN/NN/NR 与慢平移误判。
...
### 10.3 删 clone 的概率语义与固定槽复用

一次性轨迹更新已经把量测写入后验后，联合协方差分块

\[
P=
\begin{bmatrix}P_{aa}&P_{ad}\\P_{da}&P_{dd}\end{bmatrix}
\]

中丢弃 clone \(d\) 的边缘协方差就是 \(P_{aa}\)。不要再计算

\[
P_{aa}-P_{ad}P_{dd}^{-1}P_{da},
\]

因为那是“已知被删 clone 真值”时的条件协方差。只有保存信息矩阵的优化器才在边缘化时
对信息块做 Schur 补。

当前 30 槽实现不搬动 \(P_{aa}\)，只把物理槽标为空闲。空槽量测列必须为零；再次增广时
按

\[
P_{C\chi}=J_CP_{I\chi},\qquad P_{CC}=J_CP_{II}J_C^T
\]

覆盖槽位整行整列，包括持久点尾部。否则固定槽从性能选择变成相关性错误。必读：
[Clone 删除与槽位复用](CLONE_REMOVAL_AND_SLOT_REUSE.md)。

### 10.4 三自由度 landmark 参数化
...
## 15. 第 12 阶段：二轮数学审计与报告指标校正

对应范围：**3e7d6ed18495ccb735e5771f529e4601e6e148d0 到本文件所在提交**。

这一阶段不改变 ESKF/Schur 估计器，而是做一次独立的“代码公式—报告公式—专题文档”
交叉审计。审计发现并补齐四个此前没有独立闭环的主题：

1. 初始均值、\(P_0\)、首条 IMU、相机时刻零阶保持与仿真 oracle Q/P/V 的边界；
2. 协方差形式删 clone、信息形式 Schur 边缘化、条件化与固定物理槽复用的区别；
3. ATE/RPE、四自由度 gauge、联合 NEES、序贯 NIS 和 `neg_cov` 的实现级定义；
4. Rotation-translation 的解析 R/N 轨迹、姿态和特征分布。

同时修正一个只影响报告的一致性问题。滤波器使用世界系左乘姿态误差，但旧联合 NEES 用

\[
\operatorname{Log}(R_{gt}^TR_{est})
\]

与 \(P_{\theta p},P_{\theta v}\) 混合；它的角度范数虽正确，切空间却不匹配。改为

\[
e_\theta=\operatorname{Log}(R_{est}R_{gt}^T)
\]

后，Q/P/V 9 维误差与左乘协方差完全同域。轨迹 RMSE/RPE 的旋转角范数不应发生实质变化，
但 `mean_nees` 需要重新生成，不能沿用旧表。

旧 `neg_cov` 只看三个 trace 是否为负，会漏掉 trace 为正的负特征方向。当前改为对
\(P_{qpv}\) 做 9×9 谱检查：

\[
\lambda_{min}<-10^{-10}\max(1,\max_i|P_{ii}|).
\]

轨迹 CSV 额外输出最小特征值和尺度，HTML 对旧 CSV 自动回退到 trace 报警。这是更严格的
主状态健康度检查，但不是含空槽与全部持久点的完整联合矩阵谱证明。

```mermaid
flowchart TD
    C["代码中的误差状态与矩阵"] --> A["逐公式审计"]
    D["专题文档中的推导"] --> A
    R["CSV/HTML 中的指标"] --> A
    A --> I["补初始化与时间专题"]
    A --> M["补 clone 删除与槽位专题"]
...
验收要求：Release 构建通过；报告 JS 可执行；四场景轨迹误差与算法端点一致；新 NEES
有限；`neg_cov=0`；新增 Markdown 链接、公式定界符和 Mermaid 语法通过静态检查。

必读：[初始化与时间同步](INITIALIZATION_AND_TIME_SYNCHRONIZATION.md)、
[Clone 删除与槽位复用](CLONE_REMOVAL_AND_SLOT_REUSE.md)、
[评估指标与 gauge 对齐](EVALUATION_METRICS_AND_GAUGE_ALIGNMENT.md)、
[Rotation-translation 场景](SIM_ROTATION_TRANSLATION.md)。

## 16. 当前 Hpp/Hll 求解器应该怎样重建
...
- FEJ 零空间泄漏；
- 增广 cross covariance；
- 删 clone 后保留主子块、槽位复用整行列覆盖；
- 首条 IMU、相机时刻和初始先验语义；
- 左乘姿态误差与 Q/P/V 联合 NEES 坐标一致；
- \(P_{qpv}\) 最小特征值相对阈值检查；
- LDLT 与特征分解后验数值对比。
...
| 场景 | 位置 RMSE | 速度 RMSE | 姿态 RMSE | 最大位置误差 |
|---|---:|---:|---:|---:|
| Circle-out | 0.3262 m | 0.0585 | 0.00423 | 0.6316 m |
| Circle-in | 0.3124 m | 0.0531 | 0.00431 | 0.6157 m |
| Helix-3D | 0.0867 m | 0.0492 | 0.00413 | 0.3025 m |
| Stop-go | 0.0604 m | 0.0581 | 0.00578 | 0.1543 m |

| 场景 | 左乘 mean NEES | mean NIS | min \(\lambda(P_{qpv})\) | neg_cov |
|---|---:|---:|---:|---:|
| Circle-out | 1.056 | 0.982 | \(2.20\times10^{-6}\) | 0 |
| Circle-in | 0.707 | 0.955 | \(8.19\times10^{-7}\) | 0 |
| Helix-3D | 0.343 | 0.921 | \(1.12\times10^{-6}\) | 0 |
| Stop-go | 0.282 | 0.602 | \(2.40\times10^{-6}\) | 0 |

四场景 reused=0、blocked=0。Stop-go 产生 5772 个无深度旋转约束，证明弱视差降级路径
确实被覆盖。表中联合 NEES 使用 \(\operatorname{Log}(R_{est}R_{gt}^T)\)，与左乘世界系
协方差一致；Q/P/V 联合谱检查覆盖每个相机采样点。评估坐标修正不会改变轨迹，四场景
RMSE 与算法端点一致。耗时与机器负载相关，不应作为跨机器验收阈值。
...
| 数学总审计与重实现路线 | 3e7d6ed–本文件所在提交 | [INITIALIZATION_AND_TIME_SYNCHRONIZATION.md](INITIALIZATION_AND_TIME_SYNCHRONIZATION.md)、[CLONE_REMOVAL_AND_SLOT_REUSE.md](CLONE_REMOVAL_AND_SLOT_REUSE.md)、[EVALUATION_METRICS_AND_GAUGE_ALIGNMENT.md](EVALUATION_METRICS_AND_GAUGE_ALIGNMENT.md) |

## 20. 最容易“重实现成功但数学已经变了”的地方
...
17. 在协方差形式删 clone 时误做条件化 Schur 补；
18. 复用固定槽时只覆盖 195×195 主块，漏掉持久点尾部；
19. 用右乘机体系姿态误差配左乘世界系协方差做联合 NEES；
20. 看到 trace 为正就宣称完整协方差半正定；
21. 顺手加入严格 SO(3) 积分或 reset 雅可比，却没有独立 A/B。
...
- 仿真使用首帧真值 Q/P/V，不覆盖真实设备静止或运动初始化；
- `mean_nees/mean_nis` 来自单个固定随机种子的时间平均，不是独立 Monte Carlo 卡方检验；
- `neg_cov` 只严格检查 Q/P/V 9×9 主状态，不对含空槽和全部持久点的完整动态矩阵逐帧
  做昂贵特征分解。
...
- 在线算法不读取 landmark GT；
- 四场景和 100 秒回归不发散；
- Q/P/V 联合谱定义下 `neg_cov=0`，且 `cov_qpv_min_eig` 有限；
- reused=0、blocked=0；
- 默认 clone 预算 20，三角化视差门限 2°；
- 低视差轨迹可延迟、可做无深度旋转约束、最终有界清理；
- 持久点不超过 20，完整维护 \(P_{xL}/P_{LL}\)；
- 当前报告能显示轨迹、状态修正、噪声敏感度、三角化、landmark、生命周期和耗时；
- 联合 NEES 使用 \(\operatorname{Log}(R_{est}R_{gt}^T)\) 与左乘协方差同域；
- 文档明确说明仿真 Q/P/V 初始化不等于真实 VIO 冷启动；
- 删 clone 的协方差主子块语义、信息形式 Schur 与固定槽覆盖均有单独验收；
