...
4. 扫描普通轨迹，完成消费判定、三角化，并准备旋转约束、延迟或丢失归档
5. 对当前关键帧直接观测到的已有持久点执行联合 EKF 更新
6. 对成功三角化的普通轨迹及已准备的旋转因子建立 Hpp/Hpl/Hll/gp/gl
7. 对临时点做 Schur 消元，使用完整联合协方差更新导航、clone 和已有持久点
8. 对满足候选、网格和预算条件的当前轨迹执行条件初始化，追加新持久点
9. 其余合格轨迹只更新影子候选
10. 删除本次消费的普通轨迹，再删除计划边缘化的 clone
```
...
flowchart TD
    ...
    D --> E[扫描普通轨迹并生成消费计划]
    E --> H[普通轨迹三角化<br/>准备旋转、延迟或归档分流]
    H --> F{当前关键帧观测到<br/>已有持久点?}
    F -- 是 --> G[持久点联合EKF重投影更新]
    F -- 否 --> J[普通重投影与旋转约束线性化]
    G --> J
    J --> K[逐点Schur消元]
...
本节给出默认混合后端所需的公式摘要。一般两视图模型、纯旋转下深度自然约消、小平移的一阶
偏差、单位 bearing 噪声传播、有效协方差以及 `Hpp/gp` 到联合 EKF 的完整推导，见
[无深度纯旋转约束](DEPTH_FREE_ROTATION_CONSTRAINT.md)。
