...
> 上面 6 次采样恰好都命中 `tiny=31`。全程统计（见 §四）显示实际是
> **31 ± 2 的小范围波动**，平均 34.1，多出的 1~3 维同样是数值零
> （`λ/λmax ~ 1e-14`），不是被误杀的有效信息。
...
平均跳过 34.1（特征分解）/ 35.1（LDLT），比理论值多 3~4。
...
空 slot 只是让那 6 维的**数值**恒为 0，但**矩阵维度始终是固定的 198**：

```cpp
MatXX Hpp(COV_SIZE, COV_SIZE);   // COV_SIZE = 18 + 30×6 = 198，编译期常量
```

LDLT 分解、Schur 补、`setZero` 全都按 198×198 跑，
