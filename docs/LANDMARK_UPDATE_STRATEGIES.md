...
正式实验使用四个确定性场景、30 s、600 个特征、`uv_var=0.01`、过程噪声倍率 1、密度型 IMU 白噪声、偏置随机游走开启、FEJ 开启、硬投影关闭、三角化最小视差 8°。
...
综合“需要可用地图点位置”、非线性鲁棒性和架构一致性边界，默认策略设为 `Retriangulate`；`SchurBackSubstitution` 保留为更重视速度和局部 RPE 时的可选项，`Fixed` 用于严格消融，`IndependentEkf` 仅用于历史复现。
...
8°在初始几何质量、最终地图点误差和保留约束数量之间最好，因此设为默认。对其他三个场景的 5°/8°交叉检查中，Circle-in 和 Stop-go 的 ATE 分别改善约 3.7% 和 0.6%，Helix-3D 仅退化约 0.6%，RPE 基本持平。若未来加入 inverse-depth 延迟初始化，可让 3°–8° 的弱点先作为未成熟 track 存活，而不是立即作为欧氏点进入后验。
