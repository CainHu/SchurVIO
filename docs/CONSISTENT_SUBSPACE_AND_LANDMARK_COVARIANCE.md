...
```mermaid
flowchart LR
    P["先验 P = L L^T"] --> W["白化状态 dx = L z"]
    H["Schur Hs, gs"] --> W
    N["四维 gauge 基 N"] --> NW["Nz = L^-1 N"]
    W --> PROJ["Pi_z = I - QN QN^T"]
    NW --> PROJ
    PROJ --> HE["同一次特征分解得到 Vr, lambda_r"]
    HE --> GE["gs 只保留到 span(Vr)"]
    HE --> UP["状态更新也只使用 Vr"]
    GE --> UP
```

这条链路的关键不只是“做了投影”，而是 $H_s$、$g_s$、不可观基和最终 Kalman 方向先被
变换到同一个无量纲坐标，再共用一次秩判定。这样才不会出现矩阵删掉某方向而梯度仍保留
该方向，或投影器与 LDLT 各自认定不同零空间的情况。
