...
flowchart TD
    ...
    PROP --> COV["误差协方差传播 P = Phi P Phi^T + Qd"]
    CAM["CameraData"] --> AUG["当前位姿 clone 增广"]
    COV --> AUG
    AUG --> SCHED["调度与 track 生命周期判定"]
    SCHED --> TRI{"Landmark 已可靠初始化?"}
    ...
    EKF --> INJECT["误差注入 q,p,v,bg,ba 与 clones"]
    INJECT --> MAP["可选重三角化/Schur 回代/影子地图"]
...
`Q_d` 来自陀螺仪/加速度计白噪声和 bias random walk 连续谱密度的离散化。过程噪声增大时，先验更不自信，视觉后验增益通常增大；但过大会让传播轨迹和协方差膨胀。
...
视觉后验需要同时修正多个历史 clone，因此不能只复制名义位姿而忽略交叉协方差。
...
LDLT 默认开启，因为它比完整特征分解更快；近零或负方向仍由相对阈值跳过，而不是盲目求逆。
...
Landmark 后续可以固定、重三角化、Schur 回代，或进入不反馈导航状态的影子地图处理器。一次性 MSCKF 调度下，轨迹完成后立即删除，以确保同一像素观测不会再次作为独立量测。
