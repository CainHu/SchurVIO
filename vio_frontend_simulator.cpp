...
std::vector<State> VIOFrontendSimulator::generateGroundTruth() const {
    ...
    for (size_t i = 0; i < num_steps; ++i) {
        ...
        // 计算当前时刻的角度（圆周运动）
        double angle = (trajectory_speed_ / trajectory_radius_) * current.timestamp;
        ...
        Eigen::Vector3d up_dir(0.0, 0.0, -1.0);  // 向上方向
        Eigen::Vector3d right_dir = forward_dir.cross(up_dir).normalized();
...
std::vector<ImuData> VIOFrontendSimulator::generateImuData(const std::vector<State>& ground_truth) const {
    ...
    for (size_t i = 1; i < ground_truth.size(); ++i) {
        ...
        ImuData data;
        data.timestamp = static_cast<uint64_t>(curr.timestamp * 1e6);
...
std::vector<CameraData> VIOFrontendSimulator::generateCameraData(const std::vector<State>& ground_truth) const {
    ...
    for (const auto& state : ground_truth) {
        if (state.timestamp >= next_camera_time) {
            CameraData data;
            data.timestamp = static_cast<uint64_t>(state.timestamp * 1e6);
