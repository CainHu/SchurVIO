...
<section>
  ...
  <div class="grid g2">
    <div class="panel"><h3>位置误差 vs 3σ 界</h3><canvas id="cerrp" height="330"></canvas></div>
    <div class="panel"><h3>姿态误差 vs 3σ 界</h3><canvas id="cerra" height="330"></canvas></div>
    <div class="panel"><h3>速度误差 vs 3σ 界</h3><canvas id="cerrv" height="330"></canvas></div>
...
(function(){
  const bind=(id,k,f)=>{const e=document.getElementById(id);
    e.addEventListener('input',()=>{view[k]=f?f(e):+e.value; draw3D();});};
  ...
  ['showLmk','showAxes','showErr'].forEach((id,i)=>{
    ...
    e.addEventListener('change',()=>{view[k]=e.checked; draw3D();});
  });
  const cv=document.getElementById('c3d');
  ...
  cv.addEventListener('wheel',e=>{e.preventDefault();
    ...
    document.getElementById('zoom').value=view.zoom; draw3D();},{passive:false});
})();
...
// ================= 3. 先验 vs 后验 =================
(function(){
  let imp=0, sp0=0, sp1=0, sdx=0, n=U.length;
  let bigImp=0, bigTot=0;
  for(const r of U){
    if(r.errp_post < r.errp_prior) imp++;
    sp0+=r.errp_prior; sp1+=r.errp_post; sdx+=r.dxp;
    if(r.dxp>1e-5){ bigTot++; if(r.errp_post<r.errp_prior) bigImp++; }
  }
  const meanErr = T.reduce((a,r)=>a+r.err_p,0)/T.length;
  const ratio = sdx/n/meanErr;
  const cards=[
    ['更新次数', n, '相机帧 '+T.length+' 帧','' ],
    ['位置改善率', (100*imp/n).toFixed(1)+'%', '后验比先验更接近真值的比例',
      imp/n>0.6?'good':(imp/n>0.45?'warn':'bad')],
    ['平均修正量 |dx_p|', fmt(sdx/n,6)+' m', '平均位置误差 '+fmt(meanErr,4)+' m',''],
  ...
  for(const r of U){
    const good = r.errp_post < r.errp_prior;
...
linePlot('cerrp',{logY:true,series:[
  ...
  {name:'3σ 界', color:C('--bad'), dash:[6,4], data:T.map(r=>[r.t,Math.max(3*r.sigma_p,1e-9)])},
]});
linePlot('cerra',{logY:true,series:[
  {name:'姿态误差', color:C('--err'), data:T.map(r=>[r.t,Math.max(r.err_att,1e-9)]),lw:1.4},
  {name:'3σ 界', color:C('--bad'), dash:[6,4], data:T.map(r=>[r.t,Math.max(3*r.sigma_q,1e-9)])},
]});
linePlot('cerrv',{logY:true,series:[
  {name:'速度误差', color:C('--err'), data:T.map(r=>[r.t,Math.max(r.err_v,1e-9)]),lw:1.4},
  {name:'3σ 界', color:C('--bad'), dash:[6,4], data:T.map(r=>[r.t,Math.max(3*r.sigma_v,1e-9)])},
]});
...
(function(){
  ...
  const judge=(name,r,sig,err)=>{
    if(!(sig>0)) return `<div class="note bad"><b>${name}</b>：协方差 trace = ${fmt(sig,4)} <b>为负或无效</b>，协方差矩阵已失去正定性（数值问题）。</div>`;
    if(r>3) return `<div class="note bad"><b>${name}</b>：误差 ${fmt(err,4)} 超出 3σ 界 (σ=${fmt(sig,4)})，滤波器<b>过度自信</b>，协方差偏小。</div>`;
    if(r<0.01) return `<div class="note"><b>${name}</b>：误差 ${fmt(err,4)} 远小于 σ=${fmt(sig,4)}（比值 ${fmt(r,5)}），滤波器<b>过度保守</b>——协方差被严重高估，卡尔曼增益会失衡。</div>`;
    return `<div class="note ok"><b>${name}</b>：误差 ${fmt(err,4)} 与 σ=${fmt(sig,4)} 量级匹配（比值 ${fmt(r,3)}），协方差<b>可信</b>。</div>`;
  };
