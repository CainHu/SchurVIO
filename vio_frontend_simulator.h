...
private:
    ...
    double imu_gyro_bias_noise_std_ = 0.0005;  // 陀螺仪偏置随机游走密度
...
public:
    ...
    // 设置 IMU 噪声参数：前两项是连续时间白噪声密度，后两项是偏置随机游走密度。
    // 离散采样标准差分别为 density/sqrt(dt) 和 random_walk*sqrt(dt)。
    ...
    // Strict-ablation hook: false uses density/sqrt(dt); true reproduces the
    // historical density*sqrt(dt) scaling with the same deterministic samples.
